...
from bounds.bounds_so import cal_dual_opt, sdp_solver
...
def compute_loss_so(model_weights, X_test, Y_test, Logits, epsilon, FLAGS):
    ...
    # Data-independent SDP program
    for j in range(FLAGS.num_hidden):
        # Calculate W_i W_i^T
        W_i2[j] = np.outer(W_FC1[:, j], W_FC1[:, j])
    W_i2 = sigma_prime_2(0.0, beta) * W_i2

    so_term = np.zeros((num_classes, num_classes))
    
    for label in range(num_classes):
        for k in range(num_classes):
            if (k != label):
                a = (W_FC2[k]-W_FC2[label]).clip(0)
                psd_M = np.einsum('i, ijk -> jk',a, W_i2)
                print('Solving SDP for class', label, k)
                so_term[label, k] = sdp_solver(psd_M, FLAGS.dimension)
    ...
    for i in range(num_points):
        ...
        # Second-order term
        for k in range(num_classes):
            print('Example',i, 'Class',k, Logits[i, k]-Logits[i, label], grad_l1[k]*epsilon, so_term[label, k]*epsilon*epsilon*0.5)
        
        ## Compute max f(j) - f(true) after perturbation
        new_values = np.ravel(Logits[i, :]) - np.ravel(np.ones([num_classes, 1]) * Logits[i, label]) + epsilon * (grad_l1 + so_term[label] * 0.5 * epsilon)
...
def bounds_main_exp(X_test, Y_test, Epsilon, FLAGS):
    x = tf.placeholder(tf.float32, [None, FLAGS.dimension])
