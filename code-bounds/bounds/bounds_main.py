...
from bounds.bounds_so import cal_dual_opt, sdp_solver, sdp_feasibility
...
def compute_loss_so(model_weights, X_test, Y_test, Logits, epsilon, FLAGS):
    ...
    a = np.zeros((num_classes, num_classes, FLAGS.num_hidden))
    for label in range(num_classes):
        for k in range(num_classes):
            if (k != label):
                a[label, k] = (W_FC2[k]-W_FC2[label]).clip(0)
    #aW_M[label, k] = np.einsum('i, ijk -> jk',a, W_i2)
    ...
    for i in range(50, num_points):
        ...
        sigma_prime = beta * tf.nn.sigmoid(beta * (tf.matmul(x_ravel, W_FC1) + B_FC1))
        sigma_prime_ravel = tf.reshape(sigma_prime, [-1]).eval()
        ...
        """
        grad_fx = tf.matmul(tf.matmul(W_FC2, np.diag(sigma_prime_ravel)),np.transpose(W_FC1)).eval()
        ...
        sig_pri_2 = np.zeros(FLAGS.num_hidden)
        # add_Mat = np.zeros((FLAGS.num_hidden, FLAGS.dimension, FLAGS.dimension))
        # add_Sca = np.zeros(FLAGS.num_hidden)
        
        for j in range(FLAGS.num_hidden):
            # b is the max of either endpoints or sigma''(0)
            if (pre_act[j] < 0):
                if (pre_act[j] + b_0[j] < 0):
                    sig_pri_2[j] = sigma_prime_2(pre_act[j] + b_0[j], beta)
                else:
                    sig_pri_2[j] = sigma_prime_2(0.0, beta)
            else:
                sig_pri_2[j] = sigma_prime_2(pre_act[j], beta)
            # add_Sca[j] = int_sigm_pri_2 * np.square(np.sum(abs(W_FC1[:, j])))
            
        for k in range(num_classes):
            """
            psd_M = np.zeros((FLAGS.dimension, FLAGS.dimension))
            for j in range(FLAGS.num_hidden):
                a = W_FC2[k, j] - W_FC2[label, j]
                if (a > 0):
                    psd_M = psd_M + a * add_Mat[j]
            """
            psd_M = np.einsum('i, i, ijk -> jk', sig_pri_2, a[label, k], W_i2)
            ...
            """
            # Quick and loose bound on so term
            """
            for j in range(FLAGS.num_hidden):
                a = W_FC2[k, j] - W_FC2[label, j]
                if (a > 0):
                    so_term[k] = so_term[k] + a * add_Sca[j]
            """

            # Try the dual optimization problem
            """
            if (k != label):
                print('Solving dual for class', k)
                so_term[k] = cal_dual_opt(psd_M, FLAGS.dimension)
            """
            
            # Try the spectral norm approximation
            """
            if (k != label):
                so_term[k] = np.linalg.norm(psd_M, 2) * FLAGS.dimension
            """
            ...
            if (new_val > 0):
                ...
                break
                 """
        if i % 5 == 0:
            print('zo-loss at test example 50 to ',i,':', np.sum(Zo_loss)/(i+1-50))
