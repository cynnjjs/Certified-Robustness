...
##
#1;3409;0c# This program is licenced under the BSD 2-Clause licence,
...
class CarliniLi:
    def __init__(self, sess, FLAGS, 
        ...
        self.FLAGS = FLAGS
        self.grad = self.gradient_descent(sess, FLAGS)

    def gradient_descent(self, sess, FLAGS):
        def compare(x,y):
            if self.TARGETED:
                return x == y
            else:
                return x != y
        shape = (1, FLAGS.dimension)
        ...
        newimg = ((tf.tanh(modifier + simg) + 1)*1.0/2)
        
        
        with tf.variable_scope("model_weights") as scope:
            scope.reuse_variables()
            output = get_model(newimg, FLAGS)

        # output = model.predict(newimg)
        with tf.variable_scope("model_weights") as scope:
            scope.reuse_variables()
            orig_output = get_model((tf.tanh(timg) + 1)*1.0/2, FLAGS)
        ...
        # sum up the losses
        loss2 = tf.reduce_sum(tf.maximum(0.0,tf.abs(newimg-(tf.tanh(timg) + 1)/2)-tau))
        ...
        def doit(oimgs, labs, starts, tt, CONST, FLAGS):
            # convert to tanh-space
            # imgs = np.arctanh(np.array(oimgs)*1.999999 - 1)
            imgs = np.array(oimgs)*2 - 1
            imgs = np.arctanh(np.array(imgs *0.9999))
            

            starts = np.array(starts)*2 - 1
            starts = np.arctanh(np.array(starts *0.9999))
            ...
            while CONST < self.LARGEST_CONST:
                ...
                for step in range(self.MAX_ITERATIONS):
                    ...
                    # if step%(self.MAX_ITERATIONS//10) == 0:
                        # print(step,sess.run((loss,loss1,loss2),feed_dict=feed_dict))OB
                        # print(sess.run(timg, feed_dict = feed_dict))
                        # print(sess.run(newimg, feed_dict = feed_dict))
    ...
    def attack(self, imgs, targets, FLAGS):
        """
        ...
        for img,target in zip(imgs, targets):
            print("Image: ", count)
            r.extend(self.attack_single(img, target, FLAGS))
            count = count + 1
        return np.array(r)

    def attack_single(self, img, target, FLAGS):
        """
        Run the attack on a single image and label
        """

        # the previous image
        prev = np.copy(img).reshape((1, FLAGS.dimension))
        tau = 1.0
        ...
        while tau > 1./256:
            ...
            res = self.grad([np.copy(img)], [target], np.copy(prev), tau, const, FLAGS)
            if res == None:
