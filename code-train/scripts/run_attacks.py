...
from utils.load_har import load_har

# training 
# from train.train_new_eig import train 
from train.train_new_eig import train

# attacks
from attacks.attacks_main import attack
...
tf.app.flags.DEFINE_string('msave', 'results/temp', 'path to save the model weights')
tf.app.flags.DEFINE_string('lsave_dir', 'results/losses-temp', 'directory to save the losses')
...
def main(argv=None):
    ...
    """
    If the weights are already found, train just restores 
    the weights without additional training 
    """

    train(X_train, Y_train, X_test, Y_test, FLAGS)
    
    tf.reset_default_graph()

    Epsilon = np.linspace(0, 0.15, 10)


    if(FLAGS.run_gd == 1):
        FLAGS.attack_type = 'gd'
        attack(X_test, Y_test, 'gd', Epsilon, FLAGS)
        tf.reset_default_graph()


    if (FLAGS.run_fgsm == 1):
        FLAGS.attack_type = 'fgsm'
        attack(X_test, Y_test, 'fgsm', Epsilon, FLAGS)
        tf.reset_default_graph()


    if(FLAGS.run_cw == 1):
        FLAGS.attack_type = 'cw'
        attack(X_test, Y_test, 'cw', Epsilon, FLAGS)
        tf.reset_default_graph()
