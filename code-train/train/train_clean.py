...
def get_classification_loss(sess, x, y_, y_model, FLAGS):
...
# Testing speed issue
#def get_so_class_loss(x, y_, y_model, i,  grad_fx, a_pos, a_neg, sig_pri_2_max, sig_pri_2_min, w_i2, FLAGS):
def get_so_class_loss(x, y_, y_model, i, grad_fx, w_i2, FLAGS):
...
def get_regularization_loss(sess, x, y_, y_model, FLAGS):
    ...
    if(FLAGS.reg_type == 'gd'):
        ...
        adv_loss = get_classification_loss(sess, x, y_, y_adv, FLAGS)
        adv_list = []
    ...
    if(FLAGS.reg_type == 'so_pgd'):
        ...
        for r in range(0, FLAGS.num_classes):
            ...
            # Testing speed issue
            class_loss = get_so_class_loss(x, y_, y_model, r, grad_fx, w_i2, FLAGS)
...
def train(X_train, Y_train, X_test, Y_test, FLAGS):
    ...
    with tf.variable_scope("perturbation") as scope:
        #per = tf.fill(tf.stack([FLAGS.num_classes, tf.shape(x)[0], FLAGS.dimension]), 0.0)
    ...
    classification_loss = get_classification_loss(sess, x, y_, y_model, FLAGS)
...
#sess.run(tf.global_variables_initializer())
    ...
    with sess.as_default():
        ...
        if (FLAGS.reg_type == 'so_pgd'):
            ...
            foso_margin = get_regularization_loss(sess, x, y_, y_model, FLAGS)
            ...
            train_step_per = opt.minimize(-train_loss, var_list = [per])
            ...
            final_loss = get_regularization_loss(sess, x, y_, y_model, FLAGS)
        ...
        for j in range(FLAGS.restore_epoch, FLAGS.num_epochs):
            ...
            for i in range(num_batches):
                ...
                if i % 100 == 0:
                    ...
                    if(FLAGS.reg_type != 'None' and FLAGS.reg_param!=0):
                        ...
                        if (FLAGS.reg_type != 'so_pgd'):
                            ...
                            for r in range(FLAGS.num_classes):
                                Reg_loss[r] = final_loss[r].eval( feed_dict = {x: X_train[0:FLAGS.batch_size, :], y_:Y_train[0:FLAGS.batch_size, :]})
                ...
                if ((FLAGS.reg_type != 'None') and (FLAGS.reg_type != 'so_pgd')):
                    ...
                    sess.run(Train_step[i%FLAGS.num_classes], feed_dict={x: X_train[start:end, :], y_:Y_train[start:end, :]}, options = options, run_metadata=run_metadata)
                else :
                    ...
                    for i1 in range(1):
                        print("Train perturbation step:", i1)
                        # Train perturbation step
                        sess.run(train_step_per, feed_dict={x: X_train[start:end, :], y_:Y_train[start:end, :]}, options = options, run_metadata=run_metadata)
                        # Project pertubation
