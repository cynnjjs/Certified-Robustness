...
def grad_2 (x, a, beta):
    grad_2 = (beta**2) * tf.exp(-beta*(a+x)) / (x * (1.0+tf.exp(-beta*(a+x)))**2) - (sigma_prime(x+a, beta)-sigma_prime(a, beta)) / (x**2)
    #grad_2 = tf.clip_by_value(grad_2, -99999.9, 99999.9)
    return grad_2
...
def train(X_train, Y_train, X_test, Y_test, FLAGS):
    ...
    W_fc1 = np.load('pgdBeta5_weights_W_fc1.npy')
    B_fc1 = np.load('pgdBeta5_weights_B_fc1.npy')
    W_fc2 = np.load('pgdBeta5_weights_W_fc2.npy')
    B_fc2 = np.load('pgdBeta5_weights_B_fc2.npy')
    ...
    with sess.as_default():
        ...
        if (FLAGS.reg_type == 'so_pgd'):
            ...
            # Exploding Gradients???
            gvs = opt.compute_gradients(train_loss, var_list = weights_train_vars)
            capped_gvs = [(tf.clip_by_value(grad, -99999.9, 99999.9), var) for grad, var in gvs]
            train_step_weights = opt.apply_gradients(capped_gvs)
        ...
        with tf.variable_scope("model_weights", reuse = True):
            ...
            sess.run(tf.assign(w_fc1, W_fc1))
            sess.run(tf.assign(b_fc1, B_fc1))
            sess.run(tf.assign(w_fc2, W_fc2))
            sess.run(tf.assign(b_fc2, B_fc2))
        ...
        for j in range(FLAGS.restore_epoch, FLAGS.num_epochs):
            ...
            for i in range(num_batches):
                ...
                with tf.variable_scope("model_weights", reuse = True):
                    ...
                    ww2 = w_fc2.eval()
                    bb2 = b_fc2.eval()
                    print(np.argwhere(np.isnan(ww1)), np.argwhere(np.isnan(bb1)),np.argwhere(np.isnan(ww2)),np.argwhere(np.isnan(bb1)))
                    #print(w_fc1.eval()[1, 2], b_fc1.eval()[3], w_fc2.eval()[0, 1], b_fc2.eval()[1])
                ...
                if i % 1 == 0:
                    ...
                    if (FLAGS.reg_type != 'None' and FLAGS.reg_param!=0):
                        ...
                        Train_loss = train_loss.eval(feed_dict={x:X_train[start:end, :], y_:Y_train[start:end, :]})
                        Classification_loss = classification_loss.eval(feed_dict={x:X_train[start:end, :], y_:Y_train[start:end, :]})
                    ...
                    # Saving all the weights and models
                    model_path = FLAGS.msave + "-epoch" + str(j) + "-step" + str(i)+ "-model"
                    save_path = saver.save(sess, model_path)
                    print("Epoch", j, "step", i, "Model saved in file: %s" % save_path)
